...
bias_logistic_sim <- function(betaVec, nSubs, probVec,
                         nSims, checkpoint) {
...
# # test the simulation
# set.seed(0)
# test = sim_logistic(betaVec = c(-1, -0.5, 1),  nSubs=5000, probVec=c(0.98, 0.01, 0.01), nSims=5000, checkpoint=TRUE)
# apply(test, 2, mean)
# # test the solver
# test2 <- bias_logistic_nleqslv(betaVec = c(-1, -0.5, 0.9), probVec=c(0.98, 0.01, 0.01))
# test2$x
