#' powerD_meanCor.R
#' For binary outcome and uncorrealted genotypes, calculate simulated power for combined and seperated modles using SKAT, and also calculate the theoretical power using  Lee's method
...
#' # Should return powerE= 0.9999946,powerSeparated=1,powerCombined=0.3
...
powerD_mean<-function(k,n,alpha,p,runs,list1,list2){
  ...
  for (ii in seq_along(1:length(list1))) {
    ...
    for (i in 1:runs) {
      ...
      # construc G matrix for separated model
      G <- matrix(rbinom(k*n, 2, p), ncol = k)
      # combine the last two alleles
      last <- G[, k-1] + G[, k]
      last[last > 2] <- 2
      # construct G matrix for combined model
    ...
    # calculate A and B using expectation
